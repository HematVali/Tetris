...
class Button():  # all the images will follow this calss
    ...
    def draw(self,win):  # this will draw the image on the screen
        ...
        if self.rect.collidepoint(mouse_position):#checks where the butter is pressed
            if pygame.mouse.get_pressed()[0] == 1: #if it pressed than this will happen
                self.clicked = True
                pressed = True
...
def main_menu():
    ...
    while run:
        ...
        #draws the buttons
        if EasyModeButton.draw(win):#if it works this module will be loaded
            EasyModeButton.clicked = False
            main_EGame()
            run = False
        if RegularModeButton.draw(win):#if it works this module will be loaded
            main_RGame()
            run = False
        if HardModeButton.draw(win):#if it works this module will be loaded
            main_DGame()
            run = False
...
def game_over_screen():
    ...
    while run:
        win.fill((0,0,0))
        if play_again.draw(win):#if it works this module will be loaded
            main_menu()
            run = False
        if quit.draw(win):
            run = False
